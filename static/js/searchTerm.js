...
const TemplateRenderer = {
  ...
  renderField(label, value, isList = false) {
    ...
    const content = Array.isArray(value)
      ? isList
        ? `<ul class="list-none list-inside text-md">
            ${value.map((item) => `<li>${formatValue(item)}</li>`).join("")}
          </ul>`
        : value
            .map(
              (item) => `<span class="mb-0 text-md">${formatValue(item)}</span>`
            )
            .join("")
      : isList
      ? `<ul class="list-none list-inside text-md">
      ...
      : `<p class="text-md font-normal text-gray-900">${formatValue(
          value
        )}</p>`;
  ...
  renderLexicalRelations(label, relations) {
    ...
    const tableRows = relations
      .map((relation) => {
        ...
        return `
        <tr>
          <th class="pl-0 py-2 text-md font-normal">${TextFormatter.format(
            key
          )}</th>
          <td class="pl-0 py-2 text-md">${formattedValues}</td>
        </tr>`;
      })
      .join("");
